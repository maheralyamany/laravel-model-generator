...
use App\CodeGenerator\RenderableModel;
...
class ClassModel extends RenderableModel
{
    ...
    {
        return collect($this->cascadeRelations)->filter(fn($r) => ($r::class === 'App\ModelGenerator\Model\BelongsToMany'))->toArray();
    }
    /**
     *
    ...
    {
        $relarions = collect($this->cascadeRelations)->filter(fn(Relation $r) => ($r::class === 'App\ModelGenerator\Model\BelongsTo' && $r->getIsCascade()));
